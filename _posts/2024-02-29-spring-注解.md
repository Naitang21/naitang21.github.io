...
## @Import
`@Import` 是一个 Java 注解，用于在配置类中导入其他配置类、组件或bean。

使用 `@Import` 注解可以将另一个配置类或组件引入当前的配置类中，以便将它们的配置或功能合并到当前配置中。这样可以方便地组合和重用不同的配置，并将它们整合到一个统一的配置类中。


<!--stackedit_data:
eyJoaXN0b3J5IjpbMjExNDk5NDk4OF19
-->